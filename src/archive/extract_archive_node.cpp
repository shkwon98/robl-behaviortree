...
{
    if (!getInput("file", file_))
    {
        throw BT::RuntimeError("missing required input [file]");
    }

    progress_ = 0.0f;

    if (auto ext = std::filesystem::path{ file_ }.extension(); ext == ".gz")
    {
        if (ext = std::filesystem::path{ file_ }.replace_extension("").extension(); ext == ".tar")
        {
            ...
            future_ = std::async(std::launch::async, &ExtractArchiveNode::ExtractGzip, this, file_);
...
BT::NodeStatus ExtractArchiveNode::onRunning()
{
...
void ExtractArchiveNode::onHalted()
{
}

bool ExtractArchiveNode::ExtractGzip(const std::string &filename)
{
